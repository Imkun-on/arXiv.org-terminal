...
# ============================================================
# Inline replacements for Shared/logger_setup + Database_Globale
# ============================================================
...
# --- Symbols ---
SYM_OK = "[success]✓[/success]"
SYM_FAIL = "[error]✗[/error]"
...
_DB_PATH = os.path.join(_SCRIPT_DIR, 'downloads.db')
...
def record_download(*, source_id, title, source_url, file_path,
                    file_size_bytes, collection_name, collection_id,
                    artist, audio_format, **_kw) -> None:
    try:
        con = sqlite3.connect(_DB_PATH)
        con.execute(
            '''INSERT INTO downloads
               (source_id, title, source_url, file_path, file_size,
                category, meta_json, authors, fmt)
               VALUES (?,?,?,?,?,?,?,?,?)''',
            (source_id, title, source_url, file_path, file_size_bytes,
             collection_name, collection_id, artist, audio_format),
        )
        con.commit()
        con.close()
    except Exception as e:
        logging.getLogger('scraper_arxiv').debug("DB write error: %s", e)


# ============================================================
# Original script (self-contained)
# ============================================================
...
OUTPUT_DIR = os.path.join(_SCRIPT_DIR, 'download_arxiv')
...
def _display_related_papers(arxiv_id: str) -> None:
    ...
    for i, p in enumerate(related, 1):
        ...
        table.add_row(
            str(i), title_display, p['authors'][:22],
    ...
    for pdf_path in pdf_files:
        try:
            ...
            if matches:
                ...
                results.append({
                    'file': filename, 'path': pdf_path, 'arxiv_id': arxiv_id,
...
def _display_local_search(keyword: str) -> None:
    ...
    if pdf_count == 0:
        ...
        return
    with Progress(
        SpinnerColumn("dots"), TextColumn("[bold bright_cyan]{task.description}"),
    ...
    for i, r in enumerate(results[:20], 1):
        ...
        table.add_row(
            str(i), r['arxiv_id'] or '-', r['file'][:45],
            str(r['total_matches']), pages_str,
        )
    console.print()
    console.print(Panel(
        table,
...
_RSS_CATEGORIES = [
    ...
    ('stat', 'Statistics'), ('eess', 'Electrical Eng.'),
    ('physics', 'Physics'), ('astro-ph', 'Astrophysics'),
    ('cond-mat', 'Condensed Matter'), ('quant-ph', 'Quantum Physics'),
    ('hep-th', 'HEP Theory'), ('gr-qc', 'Gen. Relativity'),
    ('econ', 'Economics'), ('q-bio', 'Quant. Biology'),
    ('q-fin', 'Quant. Finance'),
]
...
def _fetch_single_rss(code: str, name: str) -> dict:
    ...
    try:
        r = requests.get(
            ...
            timeout=10, headers={'User-Agent': 'ArxivScraper/1.0'},
        )
...
def _display_today_stats() -> None:
    ...
    table = Table(
        ...
        header_style="bold bright_cyan", expand=False, show_footer=True,
    )
    ...
    for s in sorted(stats, key=lambda x: -(x['new'] + x['updated'])):
        ...
        if count >= 500:
            ...
            total_str = str(count)
        table.add_row(s['code'], s['name'], str(s['new']), str(s['updated']), total_str, sub_str)
...
def search_papers(query: str, category: str = '', max_results: int = MAX_SEARCH_RESULTS,
                  sort_by: str = 'relevance', on_progress=None) -> list[arxiv.Result]:
    ...
    if is_title_search:
        ...
        title_query = f'cat:{category} AND ti:"{query}"' if category else f'ti:"{query}"'
        ...
        title_query = f'cat:{category} AND {ti_parts}' if category else ti_parts
...
def _display_results(results: list[arxiv.Result]) -> None:
    with Progress(
        ...
        _fetch_citations_batch(results, on_advance=lambda: cit_progress.advance(cit_task))
        cit_progress.update(cit_task, description="Citations loaded!")

    table = Table(
        title=f"Results ({len(results)})",
        box=ROUNDED, border_style="bright_blue",
        header_style="bold bright_cyan", row_styles=["", "dim"], expand=False,
    )
    table.add_column("#", style="bold yellow", justify="right", width=4)
    table.add_column("Title", style="white", max_width=55, no_wrap=True)
    table.add_column("Authors", style="bright_magenta", max_width=22, no_wrap=True)
    ...
    for i, r in enumerate(results, 1):
        ...
        table.add_row(
            str(i), title_display, authors[:22], r.primary_category,
            _format_date(r.published), _format_citations(cit['citations']), pages or '-',
        )
    console.print()
    console.print(table)
...
def download_paper(result: arxiv.Result, output_dir: str,
    ...
    title = result.title
    info = {'id': arxiv_id, 'title': title, 'status': 'fail', 'file': '', 'error': '', 'size': 0}
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if fsize >= MIN_PDF_SIZE:
                ...
                record_download(
                    source_id=arxiv_id, title=title,
                    source_url=result.entry_id, file_path=filepath,
                    file_size_bytes=fsize, collection_name=result.primary_category,
                    collection_id=_json.dumps(extra_meta, ensure_ascii=False),
                    artist=authors, audio_format='pdf+latex' if download_latex else 'pdf',
                )
                return info
            else:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, result in enumerate(results):
                ...
                tid = file_progress.add_task(
                    f"[bold]#{i+1}[/bold] {arxiv_id}", total=None, visible=True,
                )
                task_ids[i] = tid
...
def _filter_today_papers(papers: list[dict]) -> list[dict]:
    ...
    console.print()
    console.print(Panel(
        filt_table, title="[bold bright_white]Filters[/bold bright_white]",
...
def _display_today_papers(papers: list[dict], max_show: int = 30) -> None:
    ...
    for i, p in enumerate(show, 1):
        ...
        table.add_row(str(i), p['title'][:55], authors[:22],
                      p['primary_category'], date_str, new_icon)
    console.print()
    console.print(table)
...
def main() -> None:
    ...
    cmd_table.add_row("[bright_green]Search[/bright_green]", "transformer", "Search papers by keyword")
    cmd_table.add_row("", "1706.03762", "Search paper by arXiv ID")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[bright_cyan]Prefixes[/bright_cyan]", "au:[bright_cyan]hinton[/bright_cyan]", "Search by author")
    cmd_table.add_row("", "ti:[bright_cyan]attention[/bright_cyan]", "Search in title")
    cmd_table.add_row("", "abs:[bright_cyan]reinforcement[/bright_cyan]", "Search in abstract")
    cmd_table.add_row("", "co:[bright_cyan]NeurIPS[/bright_cyan]", "Search in comments (conference, pages)")
    cmd_table.add_row("", "jr:[bright_cyan]Nature[/bright_cyan]", "Search by journal")
    cmd_table.add_row("", "doi:[bright_cyan]10.1038/...[/bright_cyan]", "Search by DOI")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[bright_magenta]Categories[/bright_magenta]", "cat", "Show all categories list")
    cmd_table.add_row("", "cat:[bright_cyan]cs.AI[/bright_cyan]", "Search within a category")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[bright_yellow]Today's papers[/bright_yellow]", "today", "All papers submitted today")
    cmd_table.add_row("", "today:[bright_cyan]cs[/bright_cyan]", "Today's papers by category")
    cmd_table.add_row("", "today:[bright_cyan]cs.AI[/bright_cyan]", "Today's papers by subcategory")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[bright_white]Related[/bright_white]", "related:[bright_cyan]1706.03762[/bright_cyan]", "Related papers (via Semantic Scholar)")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[bright_green]Full-text[/bright_green]", "search:[bright_cyan]attention[/bright_cyan]", "Search keyword in downloaded PDFs")
    cmd_table.add_row("", "", "")
    cmd_table.add_row("[dim]Other[/dim]", "stats", "Show today's announcement stats")
    cmd_table.add_row("", "q", "Exit program")
